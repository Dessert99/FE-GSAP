...
export function RegisteredEffectExample() {
  ...
  plugins: '${registeredEffectRegistration.plugins}',
  ...
  return (
    <div ref={scope}>
      <InteractiveExample
        ...
        preview={<div className="registered-effect-example"><div className={targetClassName}><span>NOTICE</span><strong>저장된 animation recipe</strong></div><dl><div><dt>callback targets</dt><dd>Array · {observation.targetCount}개</dd></div><div><dt>effective duration</dt><dd>{observation.effectiveDuration}초</dd></div><div><dt>direct 반환</dt><dd>{observation.returnedTween ? 'Tween' : '준비 중'}</dd></div></dl><p role="status">{status}</p></div>}
        code={code}
        propertyDetails={[
          ...
          { name: 'direct return', type: 'callback 반환 타입', defaultValue: 'effect callback 결과', acceptedValues: '이 예제는 Tween' },
        ]}
        ...
        explanation={<p><code>registerEffect()</code>는 recipe를 저장할 뿐 animation instance를 반환하지 않습니다. 이후 <code>gsap.effects[name]()</code>을 호출하면 wrapper가 targets를 array로 바꾸고 defaults를 merge한 다음 callback 반환 Tween을 그대로 돌려줍니다.</p>}
        onReplay={run}
      />
    </div>
  )
