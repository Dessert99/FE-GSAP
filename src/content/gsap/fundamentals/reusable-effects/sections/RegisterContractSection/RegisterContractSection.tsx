/** registerEffect config의 다섯 key와 callback 입력·반환 계약을 설명한다. */
import { SectionHeading } from '../../components/SectionHeading/SectionHeading'

/** registry에 저장되는 값과 호출 시점에 전달되는 값을 구분한다. */
export function RegisterContractSection() {
  return (
    <section className="reusable-effects-page__section" aria-labelledby="register-contract">
      <SectionHeading number="02" id="register-contract" title="registerEffect에는 무엇을 등록하나요?" description="등록 config는 recipe의 이름, callback, 기본값, plugin 요구사항, Timeline 확장 여부를 한 번에 정의합니다." />
      <div className="reusable-effects-page__table-wrap"><table><thead><tr><th scope="col">key</th><th scope="col">역할</th><th scope="col">기본·경계</th></tr></thead><tbody>
        <tr><th scope="row"><code>name</code></th><td><code>gsap.effects</code>의 registry key</td><td>확장하면 Timeline method 이름도 됨</td></tr>
        ...
        <tr><th scope="row"><code>plugins</code></th><td>필요한 plugin 이름을 comma-separated string으로 선언</td><td>등록되지 않은 이름은 warning</td></tr>
...
// result는 undefined — registerEffect()의 반환 타입은 void`}</code></pre>
      <p className="reusable-effects-page__note">callback에 도착한 <code>targets</code>는 selector나 단일 Element가 아니라 이미 정규화된 target array입니다. 세 번째 <code>timeline</code>은 Timeline extension으로 호출했을 때 parent Timeline을 가리킵니다.</p>
    </section>
  )
}
