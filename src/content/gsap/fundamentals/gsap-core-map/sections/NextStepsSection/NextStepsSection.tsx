...
const steps = [
  ...
  { title: 'gsap.to()', description: '현재값에서 목표값으로 가는 첫 standalone Tween', href: '/fundamentals/gsap-to' },
  { title: 'to · from · fromTo · set', description: '시작값과 도착값을 누가 정하는지 비교', href: '/fundamentals/tween-start-end-values' },
  { title: 'Tween configuration', description: '기본값과 전역·지역 설정의 적용 범위', href: null },
  { title: 'CSS · non-CSS targets', description: '대상 종류에 따라 값을 표현하는 방법', href: '/fundamentals/css-animation' },
  { title: 'Easing', description: '같은 거리와 duration에서 속도감이 달라지는 이유', href: '/fundamentals/easing' },
  { title: 'Tween · Timeline instances', description: '만들어진 animation의 상태·시간·제어 method', href: '/fundamentals/tween-playhead' },
] as const
      ...
      <ol className="core-map-page__roadmap">
        {steps.map((step, index) => (
          <li key={step.title}>
            ...
            <div>
              {step.href ? <a href={toHref(step.href)}>{step.title}</a> : <strong>{step.title}</strong>}
              <p>{step.description}</p>
            </div>
            <small>{step.href ? '읽을 수 있음' : '다음 handoff 후보'}</small>
          </li>
        ))}
      </ol>
    </section>
