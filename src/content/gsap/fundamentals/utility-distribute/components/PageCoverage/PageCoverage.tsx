...
export function PageCoverage() {
  return (
    <nav className="utility-distribute-coverage" aria-label="값 분배 학습 순서">
      <div className="utility-distribute-coverage__summary">
        <div><strong>{mappedSourceCount}/{utilityDistributeCoverage.officialSources}</strong><span>대조한 공식 문서</span></div>
        <div><strong>{officialItemCount}/{utilityDistributeCoverage.officialSourceItems}</strong><span>설명한 핵심 동작</span></div>
        <div><strong>{probeItemCount}/{utilityDistributeCoverage.probeItems}</strong><span>직접 확인한 경계</span></div>
        <p>index와 grid 거리를 값으로 바꾸는 핵심 동작을 순서대로 설명하고, 문서가 말하지 않은 조합 결과는 직접 확인해 구분했습니다.</p>
      </div>
      <ol>
        {utilityDistributeSections.map((section) => (
          <li key={section.id}>
            <a href={`#${section.id}`}>
              ...
              <div><strong>{section.title}</strong><small>{section.sourceItems}개 핵심 동작</small></div>
            </a>
          </li>
        ))}
      </ol>
