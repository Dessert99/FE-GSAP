...
export type RegistryDescriptor = {
  ...
  targetX: number
  duration: number
  /** 모션 감소 설정이 아니면 재생·일시정지 버튼을 제공한다. */
...
/** getById가 이 Tween 하나를 다시 찾는 데 사용할 고유 id다. */
...
function createDescriptor(reducedMotion: boolean): RegistryDescriptor {
  return { selector: targetSelector, tweenId, targetX, duration, allowPlayback: !reducedMotion }
}
...
export function useTweenRegistryAnimation() {
  ...
  const [status, setStatus] = useState('아직 조회하지 않았습니다. 재생하거나 헤드를 옮긴 뒤 조회 버튼을 눌러 보세요.')
  // 완료되어 registry에서 사라진 Tween도 같은 실험을 반복할 수 있게 새로 만드는 열쇠다
  const [runKey, setRunKey] = useState(0)
  // 운영체제 모션 감소 설정에서는 재생 버튼 대신 slider로만 재생 헤드를 옮긴다
  ...
  useGSAP(
    () => {
      ...
      gsap.to(box, {
        ...
        onUpdate() {
          const tween = gsap.getById<gsap.core.Tween>(descriptor.tweenId)
    ...
    // 모션 설정이 바뀌거나 초기화하면 이전 Tween을 되돌리고 재생 방식부터 다시 준비한다
  ...
  function play() {
    const tween = gsap.getById<gsap.core.Tween>(descriptor.tweenId)
    if (!tween) {
      setStatus('완료되어 registry에서 정리된 Tween입니다. Tween 새로 만들기를 눌러 다시 시작하세요.')
      return
    }
  ...
  // 조회에서 사라진 완료 Tween도 같은 조건으로 다시 관찰할 수 있게 재생성한다
  function reset() {
    setRunKey((key) => key + 1)
    setStatus('Tween을 새로 만들었습니다. 다시 재생하거나 조회해 보세요.')
  }

  // TSX가 controls·조회 패널·코드 패널을 같은 descriptor에서 그리도록 필요한 값만 전달한다
  return { scope, descriptor, progress, query, status, reducedMotion, runQuery, play, pause, seek, reset }
}
