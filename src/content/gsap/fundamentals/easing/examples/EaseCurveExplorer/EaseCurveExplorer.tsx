...
export function EaseCurveExplorer() {
  ...
  const { scope, targetClassName, family, setFamily, variant, setVariant, progress, setProgress, descriptor, samples, graphSamples, currentValue, replay } = useEaseCurveAnimation()
  // runtime sample을 SVG 좌표 문법으로만 직렬화한다.
  const graphPoints = graphSamples.map((sample) => `${sample.progress * 240},${100 - sample.value * 100}`).join(' ')
    ...
    <div ref={scope}>
      <InteractiveExample
        ...
        preview={
          <div className="ease-curve-explorer">
            ...
            <svg className="ease-curve-explorer__graph" viewBox="0 0 240 100" role="img" aria-labelledby="ease-curve-title ease-curve-description">
              ...
              <desc id="ease-curve-description">가로축 progress와 세로축 value의 관계를 촘촘한 계산 지점으로 연결한 그래프</desc>
        ...
        changes={[
          ...
          'none은 대각선과 겹치고, 다른 family는 중간 progress에서 target 위치가 달라집니다.',
        ]}
        watchFor={[
          'out과 in을 바꿨을 때 곡선이 시작과 끝 중 어느 쪽에서 더 크게 휘는지 봅니다.',
          'back·elastic은 방향에 따라 value가 0 아래나 1 위로 벗어나 시작 또는 목표 위치를 잠시 넘을 수 있습니다.',
        ]}
        explanation={
