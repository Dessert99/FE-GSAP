...
// vars 배열을 먼저 두고 백분율 객체 형식을 다음 선택지로 둔다.
...
function serializeAnimationConfig(animationConfig: KeyframesAnimationConfig) {
  if (Array.isArray(animationConfig.keyframes)) {
    // 배열의 각 to-style vars를 표시할 코드 행으로 바꾼다.
...
/** vars 배열과 백분율 객체가 중간 상태의 시점을 정하는 방식을 비교한다. */
  ...
  return (
    <div ref={scope} id="keyframes-example">
      <InteractiveExample
        ...
        description="각 단계의 시간을 직접 적는 vars 배열과 전체 시간에서 상태 위치를 정하는 백분율 객체를 비교합니다."
        sourcePath="src/content/gsap/fundamentals/gsap-to/examples/KeyframesExample/useKeyframesAnimation.ts"
        reducedMotion={reducedMotion}
        controls={<div className="interactive-example__control-list"><label className="interactive-example__control"><span className="interactive-example__control-heading"><span>keyframes 형식</span></span><select value={mode} onChange={(event) => setMode(event.target.value as typeof mode)}>{modes.map((value) => <option key={value} value={value}>{value === 'array' ? '단계별 vars 배열' : '전체 시간의 백분율 객체'}</option>)}</select></label></div>}
        ...
        changes={[mode === 'array' ? '각 vars가 작은 to()처럼 순서대로 이어지고 단계마다 적은 duration을 사용합니다.' : `전체 ${'duration' in animationConfig ? animationConfig.duration : 0}초의 25%, 60%, 100% 지점에 상태를 배치합니다.`]}
        watchFor={['x만 적은 단계에서는 이전 y와 rotation 값이 유지되는지 봅니다.', '같은 target에 반복한 gsap.to() 호출을 keyframes 하나로 바꿔 순서를 한곳에서 읽는지 확인합니다.']}
        explanation={<p>vars 배열은 각 항목을 to-style 상태로 이어 붙여 같은 target의 반복 호출을 줄입니다. 그래서 연속 상태를 한 설정에서 읽기 쉽고 CSS <code>@keyframes</code> 애니메이션도 GSAP 생명주기로 옮기기 쉽습니다. 백분율 객체는 Tween의 전체 <code>duration</code>을 기준으로 각 상태의 시점을 배치합니다.</p>}
        onReplay={replay}
      />
    </div>
  )
