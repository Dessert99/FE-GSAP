/** P45 simulator와 navigation boundary를 header·example·source section으로 조립한다. */
...
/** responsive condition cleanup과 navigation ownership을 host scroll 없이 설명한다. */
  ...
  return (
    <article className='scroll-trigger-responsive-page'>
      <header>
        ...
        <p>
          <code>{scrollTriggerResponsiveMeta.sourcePath}</code> · 공식 대조일{' '}
          {scrollTriggerResponsiveMeta.reviewedAt} · official coverage 4 / 4
        </p>
      </header>
      <ResponsiveRestorationLab />
      <section aria-labelledby='responsive-boundary-title'>
        <p className='scroll-trigger-responsive-page__eyebrow'>
          OWNERSHIP BOUNDARY
        </p>
        ...
        <p>
          rendered docs는 <code>clearMatchMedia()</code>가 associated trigger와
          animation을 kill하지 않는다고 설명하지만 installed 3.15 source는
          matching context를 kill합니다. 이 version boundary를 추측으로 합치지
          않고, page-local simulator에서는 legacy API를 호출하지 않습니다.
        </p>
        <p>
          <code>clearScrollMemory()</code>는 recorded ScrollTrigger positions를
          지우고 optional history restoration 값을 바꿀 수 있습니다. route
          page는 이를 호출하지 않으며 application navigation owner가 history
          policy와 함께 결정합니다.
        </p>
      </section>
    </article>
