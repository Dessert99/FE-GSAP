...
// select에 보여줄 값과 GSAP overwrite 모드를 같은 순서로 고정한다.
const overwriteValues = ['false', 'auto', 'true'] as const

/** 겹친 Tween의 property 충돌 범위를 실행 설정과 함께 설명한다. */
export function OverwriteExample() {
  // controls·preview·표시 코드가 공유할 실제 Tween 상태를 받는다.
  ...
  // 모션 설정에 따라 학습자가 관찰할 시간 기반 차이의 범위를 설명한다.
  ...
  // 선택한 overwrite 모드가 실제 화면에서 바꾸는 결과를 요약한다.
  ...
  // 재생 중 어떤 속성의 지속 여부를 비교해야 하는지 안내한다.
  ...
  // 시간 동작을 볼 수 있는지에 맞춰 overwrite의 적용 시점을 풀이한다.
...
gsap.to('.box', {
  ...
  ease: '${overwriteTweenConfig.ease}'
        ...
        propertyDetails={[{ name: 'overwrite', type: '공식 gsap.to(): boolean | "auto"', defaultValue: '공식 gsap.to(): false', acceptedValues: '공식 gsap.to(): false · true · "auto"' }]}
        changes={changes}
        watchFor={watchFor}
