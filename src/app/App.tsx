...
import { getTrackHref, resolveRoute, toAppPath, toHref, tracks } from './routes'
...
export function App() {
  ...
  useEffect(() => {
    ...
    if (toAppPath(window.location.pathname) !== route.canonicalPath) {
      window.history.replaceState(null, '', toHref(route.canonicalPath))
    }
  }, [route.canonicalPath])
  ...
  const navigate = (path: string) => {
    if (toAppPath(window.location.pathname) !== path) {
      window.history.pushState(null, '', toHref(path))
    }
