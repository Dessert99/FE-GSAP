...
- `SRC-MSW2` — **PASS**: Modifiers·Snap·snap·wrap·wrapYoyo 공식 페이지를 다시 조회했다.
- `RDS-MSW2` — **PASS**: 각 lab의 runtime descriptor·snapshot·표시 코드를 정적으로 대조해 BLOCK이 없었다.
- `PED-MSW2` — **ADDRESSED**: 첫 화면의 `source item`·`기술 item`을 값 보정·눈금·순환의 핵심 동작과 직접 확인한 경계로 바꿨다.
- `BROWSER-MSW2` — **DEFERRED**: 실제 control·키보드·반응형·motion 조작은 수행하지 않았다.
