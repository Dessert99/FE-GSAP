...
- `SRC-GU2` — **PASS**: gsap.utils·Utility Methods 공식 페이지를 다시 조회했다.
- `RDS-GU2` — **PASS**: 각 lab의 runtime descriptor·snapshot·표시 코드를 정적으로 대조해 BLOCK이 없었다.
- `PED-GU2` — **ADDRESSED**: 첫 화면의 `source`·`기술 item`을 공식 문서·핵심 동작과 직접 확인한 경계 표현으로 바꿨다.
- `BROWSER-GU2` — **DEFERRED**: 실제 control·키보드·반응형·motion 조작은 수행하지 않았다.
