...
먼저 **"바꿀 수 있는 것은 스위치 두 개뿐"**이라는 멘탈 모델을 세운다 — `paused`(시간이 흐르는가)와 `reversed`(어느 쪽으로 흐르는가). `isActive()`는 스위치가 아니라 두 스위치와 playhead 위치로 **계산되는 결과**이며, 두 스위치와 달리 조상 timeline까지 본다. 이 모델을 세운 뒤에야 나머지 메서드가 "그 스위치를 어떻게 건드리는가"로 정리된다.
...
- reviewedAt: `2026-08-04` (최초 전수 조회) · `2026-08-05` (원문 인용 강제 재조회로 확정)
...
**authority는 `src/content/gsap/fundamentals/tween-playback-controls/tween-playback-controls.catalog.ts`다.** 66개 공식 item과 9개 실행 확인 item의 전체 문장·source·section 귀속이 그 파일 한 곳에 있다. 이 문서는 분포만 고정한다.
...
| --- | --- | --- | --- | --- |
| `playback-model` | 재생 상태는 스위치 두 개로 정해진다 | 14 | 1 | ISA 5 + PSD 6 + RVD 3 |
| `stop-and-go` | 멈추고 다시 움직이기 | 20 | 2 | PAU 7 + PLY 9 + RSM 4 |
| `restart-from-start` | 처음으로 되감고 다시 재생하기 | 6 | 2 | RST 6 |
| `reverse-direction` | 방향을 뒤집기 | 12 | 3 | RVS 12 |
| `state-getters` | 같은 이름으로 읽기도 하고 쓰기도 한다 | 14 | 1 | ISA 4 + PSD 5 + RVD 5 |
| `boundaries` | 여기서 다루지 않는 것 | 0 | 0 | 경계 선언 전용 |
| **합계** | | **66** | **9** | |
...
| module | 목적 | 근거 source |
| --- | --- | --- |
| 개념·가이드 (`CI`) | 재생 상태를 "스위치 2 + 계산값 1"의 상태 기계로 세우고, 조상 timeline 경계를 설명한다. | 여덟 source |
...
| `flow:complete-state` | 끝나면 자동으로 멈춤 상태가 되나요? | PSD-08, PB-08 |
| `flow:initial-paused` | 처음부터 멈춘 채로 만들려면? | PSD-10 |
| `flow:stop` | 지금 멈추고 싶은데 어디서 멈추나요? | PAU-01~07 |
| `flow:play-vs-resume` | 다시 움직이는 방법이 왜 둘인가요? | PLY-02, PLY-03, RSM-02, PB-04 |
...
| `flow:from-zero` | 처음부터 다시 보려면? delay는 또 기다리나요? | RST-01~06, PB-06, PB-07 |
...
| `flow:reverse-state` | reverse()를 부르면 상태가 어떻게 되나요? | RVS-05, RVS-10, PB-01, PB-03 |
...
| `flow:read-only` | isActive()는 왜 값을 못 넣나요? | ISA-01, ISA-02, PB-09 |
...
| --- | --- | --- |
| PSD-02, RVD-02 | `PlaybackModelSection.tsx` 두 스위치 비교표 "정하는 것" 열 | covered |
| PSD-05, PSD-06, RVD-03, RVD-04 | `PlaybackModelSection.tsx` 스위치 표 "조상 timeline을 보나?" 열 + 경고 블록 | covered |
| PSD-07 | `PlaybackModelSection.tsx` "멈춘다고 부모에서 빠지지는 않습니다" note 첫 문단 | covered |
| PSD-08 | `PlaybackModelSection.tsx` "끝까지 재생돼도 멈춤 스위치는 그대로입니다" 블록 | covered |
| PSD-10 | `PlaybackModelSection.tsx` 스위치 표 "처음 값 주기" 열 | covered |
...
| PB-01, PB-02, PB-03 | `ReverseDirectionSection.tsx` probe 블록 3개; `DirectionFlipLab` 관찰 패널 | covered (probe) |
| PB-04, PB-05 | `StopAndGoSection.tsx` probe 블록 2개; `StopAndGoLab` reversed 관찰값 | covered (probe) |
| PB-06, PB-07 | `RestartSection.tsx` probe 블록 2개 | covered (probe) |
| PB-08 | `PlaybackModelSection.tsx` 완료·paused 블록 provenance 문단 | covered (probe) |
| PB-09 | `StateGetterSection.tsx` "isActive()에는 쓰기 얼굴이 없습니다" 블록 | covered (probe) |

`PB-*` 9개는 공식 item이 아니다. coverage 분모(66)에 포함하지 않으며 `PageCoverage`도 공식 66개와 분리해 센다.
...
- goal: 방향 스위치를 뒤로 둔 하나의 Tween에 `resume()`과 `play()`를 각각 눌러, 멈춤은 똑같이 풀리는데 방향만 갈리는 것을 관찰한다.
...
- runtimeOwnership: hook이 고정 duration·점프 시각을 담은 단일 descriptor, `paused: true`로 만든 Tween 하나, `onUpdate`/`onComplete`/`onReverseComplete`가 Tween에서 직접 읽은 `paused()`·`reversed()`·`isActive()`·`time()`·`progress()` 관찰값, 마지막 command를 소유한다. scoped `useGSAP` 한 개, dependency 없이 한 번만 생성.
- displayOwnership: TSX가 command → 호출 문법 매핑과 descriptor·observation 직렬화, controls·관찰 패널·학습 문단을 소유한다. 상태를 다시 계산하지 않는다.
- coveredSourceItemIds: 관찰 근거로 PLY-03, RSM-02, PAU-03, PLY-04, PB-04를 보강한다(섹션이 1차 근거).
- accessibility: native button·labeled range + 연결된 `<output>`, 상태는 polite live region(`role="status"`) 하나이며 **매 프레임 바뀌는 `time`은 넣지 않는다**, 관찰 `<dl>`의 `time · progress` 칸은 `<output>`이 아닌 일반 요소, 작은 화면 1열 전환.
...
- runtimeOwnership: hook이 고정 duration·배치 시각·음수 인자 값을 담은 단일 descriptor, `paused: true` Tween 하나, Tween에서 직접 읽은 관찰값, 마지막 command를 소유한다. scoped `useGSAP` 한 개, dependency 없이 한 번만 생성.
- displayOwnership: TSX가 command → 호출 문법 매핑과 직렬화, 버튼 라벨·관찰 패널·학습 문단을 소유한다. `time` 값을 계산하지 않고 Tween이 보고한 값만 표시한다.
...
- coveredSourceItemIds: 관찰 근거로 PSD-03, PSD-09, RVD-07, ISA-02를 보강한다(섹션이 1차 근거). `isActive(true)`는 TypeScript signature에 인자가 없어 버튼으로 만들지 않고 `PB-09`를 섹션 텍스트로만 다룬다.
- accessibility: native button, 반환값 표시는 live region이 아닌 일반 요소(아래 `role="status"` 문단이 같은 내용을 한 번만 낭독한다), 관찰 `<dl>`의 `time()` 칸은 `<output>`이 아닌 일반 요소, 작은 화면 1열 전환.
...
| SRC-CORE13-004 | ADVISORY | 페이지가 표시하는 `reviewedAt`은 `2026-08-04`(최초 전수 조회일)이나, manifest가 최종 확정된 것은 2026-08-05의 인용 강제 재조회다. | 표시 대조일이 최종 확정일보다 하루 이르다 | 소유자가 원하면 `tween-playback-controls.meta.ts`의 `reviewedAt`을 `2026-08-05`로 올린다. 이 handoff는 두 날짜를 모두 남긴다 |
...
| LT-CORE13-001 | PASS | 여덟 메서드를 문서 순서대로 번역하지 않고 "스위치 둘 + 계산값 하나" 모델을 먼저 세운 뒤 의도별 4단계로 재배치했다. `play()`/`resume()`은 공식 두 문장을 나란히 놓은 대조로, getter/setter는 `paused(!paused())`를 안에서부터 읽는 해설로, `reverse(0)`은 "되감는 입장에서 0이 출발선"이라는 재해석으로 세웠다. 용어(playhead·instance·self)를 사용 전에 01단계에서 정의했다. | Learning Transformation 통과 | none |
| PROBE-CORE13-001 | PASS | 9개 probe item 전부 **측정 방법과 재현 조건**을 페이지 본문에 함께 적었다(사용한 duration, 출발 상태, 읽은 값, 반올림 자리, 실시간 측정의 프레임 오차). 단정 주장 앞에서는 선택지를 전수로 돌려 예외를 확인했다 — 아래 `verificationEvidence` 참고. | 재현 불가능한 숫자 없음 | none |
| PROBE-CORE13-002 | PASS | "항상 N이다" 형태의 주장 3건을 전수 확인했다. `reverse()`의 reversed 전환은 시작·중간·완료 3지점 전부, `restart()`의 스위치 초기화는 정방향·역방향·멈춤 3출발점 전부에서 예외가 없었다. timeScale 0 tip은 6경로 중 5경로가 동일하고 **1경로에서 예외**(`1e-8`)가 나와 본문에 예외로 명시했다. | 예외를 숨긴 단정 없음 | none |
| RDS-CORE13-001 | PASS | 세 lab 모두 hook의 descriptor·observation에서 GSAP 호출과 표시 코드가 함께 파생된다. 관찰값은 전부 `tween.paused()`·`tween.reversed()`·`tween.isActive()`·`tween.time()`을 직접 호출해 읽었고, `StateReadoutLab`의 getter/setter 판정은 반환값을 `=== tween`으로 실제 비교한 결과다. TSX 어느 파일도 `gsap`/`@gsap/react`를 import하지 않음을 grep으로 확인했다(0건). | Runtime/Display Sync 통과 | none |
...
| A11Y-CORE13-001 | ADDRESSED | 세 lab의 `role="status"` 문단에 매 프레임 바뀌는 `time`이 들어 있었고, 관찰 `<dl>`의 시간 칸이 `<output>`(암시적 `role="status"`)이었으며, `StateReadoutLab`은 반환값 `<output aria-live="polite">`와 상태 문단이 같은 내용을 이중으로 발화했다. | 재생 중 스크린리더가 프레임마다 낭독되는 결함 | live region에서 `time` 제거, 연속 변화 칸을 `<output>`이 아닌 일반 요소로 교체, 중복 live region 제거. 이산 상태(`paused`/`reversed`/`isActive`)는 `<output>`과 `role="status"`에 유지 |
...
- **공식 원문 대조** — 2026-08-04 여덟 canonical URL 직접 조회, 2026-08-05 전부 재조회. 2차 조회는 "`<메서드명>` heading부터 본문 끝까지 줄 단위로 그대로 덤프하고, (a) Parameters (b) 코드 예제 (c) note/tip/warning (d) Returns 줄 (e) 데모·관련 링크·추가 사용법을 각각 원문 인용 또는 `NOT PRESENT`로 답하라" 형식으로 강제했다. 이 재조회에서 item 5개가 되살아났다(`SRC-CORE13-002`).
- **runtime probe** — GSAP `3.15.0`(`node_modules/gsap`), Node `v22.21.0`. 공통 재현 조건: 별도 표기가 없으면 target은 평범한 객체이고 Tween은 `gsap.to({x:0},{x:100,duration:1,paused:true})`. playhead는 ticker에 의존하지 않고 `progress()`/`pause(atTime)`로 배치하며, 상태는 호출 **직후 동기적으로** `paused()`·`reversed()`·`isActive()`·`time()`·`totalTime()`·`timeScale()`을 읽었다. 시간 값은 소수 넷째 자리 반올림(허용 오차 ±0.0001). 실제 시간을 흘리는 측정만 `setTimeout`을 쓰며 앞뒤로 한 프레임(약 16ms) 오차가 있다.
  ...
  - **PB-03 완료 지점 `reverse()`** — `progress 1`에서 호출 직후 `time=1`(끝 유지), `isActive`가 `false`→`true`. `duration 0.3`으로 되감기를 끝까지 흘린 뒤에는 `time=0, progress=0, reversed=true, paused=false, isActive=false`.
  - **PB-04 `paused(false)` vs `resume()` vs `play()`** — `progress 0.5`에서 `reverse()` 후 `pause()`한 상태(`paused=true, reversed=true, timeScale=-1`)에서 각각 호출. `paused(false)`와 `resume()`은 `reversed=true, timeScale=-1` 유지, `play()`만 `reversed=false, timeScale=1`. 앞의 둘은 관측 범위에서 구분되지 않음.
  ...
  - **PB-06 `restart()` 이후 상태(전수)** — 정방향 진행 중 / `reverse()` 뒤 / `pause()` 뒤 3출발점 전부에서 호출 직후 `paused=false, reversed=false, time=0, timeScale=1`. 예외 없음. 반환값은 `=== tween`.
  - **PB-07 `restart(includeDelay)`** — `delay:1, duration:2`에서 `restart()`·`restart(false)`는 `startTime()=0`, `restart(true)`는 `startTime()=1`. 세 경우 모두 `totalTime()=0`이라 이 값으로는 구분 불가. 실시간 확인(`delay:0.4, duration:0.4`): `restart(false)`는 200ms 뒤 `progress=0.505`, `restart(true)`는 같은 시점 `0`이고 600ms 뒤 `0.5`.
  - **PB-08 완료와 paused(양방향)** — `paused(true)`인 채 `progress(1)`로 끝까지 보내면 `paused()=true` 유지. 재생해 자연 완료(`duration 0.2`, 500ms 대기)시키면 `paused()=false` 유지, `isActive()=false`. 완료는 어느 쪽으로도 값을 바꾸지 않는다.
  - **PB-09 반환값 정체성** — `pause()`·`play()`·`resume()`·`restart()`·`reverse()`·`paused(true)`·`reversed(true)` 전부 `=== tween`이 `true`. `paused()`·`reversed()`·`isActive()`는 `typeof` `boolean`. `isActive(true)`는 인자를 무시하고 그 시점 Boolean(`false`)을 반환하며 상태를 바꾸지 않는다.
...
- **타입 검사** — `npx tsc --noEmit`, `tween-playback-controls` 경로 오류 0건.
