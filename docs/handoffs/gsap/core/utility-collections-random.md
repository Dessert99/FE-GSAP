...
- `SRC-UCR2` — **PASS**: random·shuffle·toArray 공식 페이지를 다시 조회했다.
- `RDS-UCR2` — **PASS**: 각 lab의 runtime descriptor·snapshot·표시 코드를 정적으로 대조해 BLOCK이 없었다.
- `PED-UCR2` — **ADDRESSED**: 첫 화면의 `coverage`·`기술 항목`·`실행 probe`를 배열 정규화·선택·섞기 순서와 직접 확인한 경계로 바꿨다.
- `BROWSER-UCR2` — **DEFERRED**: 실제 control·키보드·반응형·motion 조작은 수행하지 않았다.
