...
- Plugin P20 구현 기준 커밋: `e4a93c9` (`P01`~`P20` 완료)
...
- Plugin 학습 페이지: **20/46**, canonical source **86/205**
- 전체 visible learning page: **60/86**, canonical source **245/364**
- Core 40개 모두 content 폴더·page handoff·lazy import·lesson 등록이 존재한다.
- Core 40개와 Plugin P01~P20 page handoff의 `releaseDecision`은 모두 `PASS`다.
- 현재 route lesson 순서는 Core 40개 뒤에 curriculum의 P01 `plugins`부터 P20 `motion-path-helper`까지 이어진다.
- 이 handoff 갱신 전 현재 HEAD에서 `npx tsc --noEmit`, `npm run build`(1042 modules), `npm run build-storybook`(1180 modules)을 실행해 모두 exit 0을 확인했다.
...
- Plugin P01~P20 중 P10 static page를 제외한 19개도 browser-only `DEFERRED`를 남겼다. Core 34개를 합친 실제 유예 page handoff는 총 53개다. P10 문서는 “DEFERRED 없음”이라는 부정 문구 때문에 단순 `rg -l DEFERRED`에는 포함된다.
...
5. `docs/handoffs/gsap/_program/session-handoff-2026-08-08-plugin-p07-p20.md`
...
| field | value |
| --- | --- |
| learningPageId | `plugin:P21` |
| slug | `motion-path` |
| route | `/fundamentals/motion-path` |
| title | `MotionPath fundamentals: move and orient along a path` |
| primary source | `source:motion-path` |
| owned canonical count | `1` |
| prerequisites | gsap.to, SVG/coordinate basics, plugin registration; P20 helper is related tooling |
| local path decision | `src/content/gsap/svg/motion-path/` |
| handoff decision | `docs/handoffs/gsap/svg/motion-path.md` |

다음 병렬 wave는 `P21 motion-path`(1 canonical), `P22 motion-path-data`(6), `P23 motion-path-coordinates`(4)다. page-local 조사는 병렬로 진행할 수 있지만 route·build·commit은 P21→P22→P23 순서로 통합한다.
...
1. 완료된 route 순서와 P01~P20 prerequisite를 먼저 재현한다.
2. 다음 wave는 P21·P22·P23의 source 조사와 page-local 구현을 최대 3개 worker에 배정할 수 있다.
3. worker는 중간에 전체 build를 요청하지 않고, 모든 import가 존재하며 대상 TypeScript가 통과하는 체크포인트에서 source 쓰기를 멈춘다.
4. 루트는 curriculum 순서대로 P21 하나만 검수·route 등록·두 build·handoff PASS·commit한다.
5. P21 commit 뒤 P22, 그다음 P23을 같은 절차로 하나씩 수용한다.
...
현재 Core page delivery는 40/40, Plugin은 20/46이야. P01 plugins부터 P20
motion-path-helper까지 route·두 build·handoff PASS·페이지 커밋이 끝났어.
시작 전에 route가 curriculum의 P01→P20 순서인지와 스무 handoff
releaseDecision PASS를 재검산해.

다음 병렬 wave는 plugin:P21 motion-path(1 canonical), P22
motion-path-data(6), P23 motion-path-coordinates(4)이야. master inventory와 plugin
curriculum의 source ownership을 우선하고, P18~P20이 이미 소유한 MorphSVG path
data와 MotionPathHelper editor lifecycle 경계를 중복하지 마.
...
root는 P21, P22, P23 순서로 하나씩 검수·통합·두 build·commit해서 동시 수정과
라우트 fallback 사고를 막아.
