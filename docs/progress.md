...
- 마지막 업데이트: 2026-08-01
- 완료: 새 3트랙 구조, 클라이언트 route 전환, 고정형 원형 목차, 웹 코드 표시 제거, CSS 책임 분리, Vitest 환경, 첫 `gsap.to()` 수직 슬라이스, 예제별 GSAP 공식 문서 링크, 프로젝트 구조 문서와 필수 참조 연결, 공용 컴포넌트 Storybook, 역할별·컴포넌트별 폴더 분리와 배치 규칙 문서화
...
- [x] `demo`·`navigation` 역할 분류와 컴포넌트별 파일 응집·배치 규칙
