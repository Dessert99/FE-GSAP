...
│                       ├── <ExampleName>.tsx
│                       ├── <ExampleName>.css
│                       └── <실행 source>        # Animation Hook·Runtime Hook·순수 example 중 선택
...
- 실행 상태와 GSAP 호출을 가진 예제는 `examples/<ExampleName>/`으로 분리하고, GSAP 실행 책임은 같은 폴더의 예제 유형별 실행 source에 둔다.
...
- Tween·Timeline·Plugin animation은 `use<ExampleName>Animation.ts`, 상태형 비애니메이션 utility는 `use<ExampleName>Runtime.ts`, 순수 호출은 `<ExampleName>.example.ts`를 사용한다. 실행 코드가 없으면 `runtimeSource: none`으로 두고 runtime 파일을 만들지 않는다.
- `<ExampleName>.tsx`는 controls, preview, runtime state 기반 code serializer, `propertyDetails`, `changes`, `watchFor`, explanation을 포함한다.
- 실행 source에는 제목·설명·속성 표·관찰점 같은 학습 패널을 넣지 않고, TSX에는 GSAP 실행 생명주기를 넣지 않는다.
- 컨트롤 값, 실제 GSAP 입력, 화면에 보이는 코드는 동일한 runtime state와 정규화 config나 discriminated descriptor에서 파생한다. TSX serializer는 그 실행값을 코드 문법으로만 포맷하고 의미를 다시 조립하지 않는다.
- 예제별 실행 source를 유지한다. `<ExampleName>.guide.md`로 실행 경계를 대신하거나 여러 예제를 공용 generic runtime에 합치지 않는다.
...
- 예제 화면의 `sourcePath`는 React 패널로 고정하지 않는다. 실행 source가 없으면 정적 코드를 소유한 TSX를 가리키거나 `none`과 이유를 기록한다.
